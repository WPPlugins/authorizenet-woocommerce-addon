...
include(plugin_dir_path( __FILE__ )."AuthorizeNet.php");
	...
	class WC_Authorizenet_Gateway extends WC_Payment_Gateway 
	{
		...
		/*Start of credit card form */
  		public function payment_fields() {
  			echo apply_filters( 'wc_authorizenet_description', wpautop(wp_kses_post( wptexturize(trim($this->authorizenet_description) ) ) ) );
			$this->form();
		}
		...
		$customer = apply_filters( 'woocommerce_authorizenet_customer_object', $customer, $wc_order );
		...
		if ($response) 
		{
			...
			if('yes' == $this->authorizenet_meta_cartspan)
			{
				...
				add_post_meta( $order_id, '_authorizenet_metas_for_cartspan', $authorizenet_metas_for_cartspan);
			}
...
function authorizenet_capture_meta_box_action($order_id, $items )
{
	if(isset($items['_authorizenet_capture_charge']) && (1 ==$items['_authorizenet_capture_charge']) ) 
	{
		...
		{
			$authorizenetpg = new WC_Authorizenet_Gateway();
		}
